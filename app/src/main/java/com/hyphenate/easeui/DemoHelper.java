...
import com.zpf.oillogistics.activity.MainActivity;
    ...
    protected void setEaseUIProviders() {
        ...
        easeUI.getNotifier().setNotificationInfoProvider(new EaseNotificationInfoProvider() {
            ...
            public Intent getLaunchIntent(EMMessage message) {
                getUserDetails(message.getFrom());
                // you can set what activity you want display when user click the notification
                Intent intent = new Intent(appContext, FriendsChatActivity.class);
...
//                    intent = new Intent(appContext, VoiceCallActivity.class);
                    ...
                    if (chatType == ChatType.Chat) { // single chat message
                        ...
                        intent.putExtra("chatType", EaseuiConstant.CHATTYPE_SINGLE);
                    } else { // group chat message
                        // message.getTo() is the group id
                        intent.putExtra("userId", message.getTo());
                        if (chatType == ChatType.GroupChat) {
                            intent.putExtra("chatType", EaseuiConstant.CHATTYPE_GROUP);
                        } else {
                            intent.putExtra("chatType", EaseuiConstant.CHATTYPE_CHATROOM);
                        }
        ...
        connectionListener = new EMConnectionListener() {
            ...
            public void onDisconnected(int error) {
                ...
                if (error == EMError.USER_REMOVED) {
                    onUserException(EaseuiConstant.ACCOUNT_REMOVED);
                } else if (error == EMError.USER_LOGIN_ANOTHER_DEVICE) {
                    Intent intent = new Intent();
                    intent.setAction("com.zpf.oillogistics.action.login");
                    intent.putExtra("content", "您的账号已在其他设备登录,请重新登录");
                    final Context mContext = AppManagerUtil.getInstance().getApplication().getApplicationContext();
                    mContext.sendBroadcast(intent);
                    LogUtil.e("--------发送广播了-----------");
                    return;


//                    onUserException(EaseuiConstant.ACCOUNT_CONFLICT);
                } else if (error == EMError.SERVER_SERVICE_RESTRICTED) {
                    onUserException(EaseuiConstant.ACCOUNT_FORBIDDEN);
                } else if (error == EMError.USER_KICKED_BY_CHANGE_PASSWORD) {
                    onUserException(EaseuiConstant.ACCOUNT_KICKED_BY_CHANGE_PASSWORD);
                } else if (error == EMError.USER_KICKED_BY_OTHER_DEVICE) {
                    onUserException(EaseuiConstant.ACCOUNT_KICKED_BY_OTHER_DEVICE);
                }
            }
    ...
    class MyGroupChangeListener implements EMGroupChangeListener {
        ...
        public void onInvitationReceived(String groupId, String groupName, String inviter, String reason) {
            ...
            broadcastManager.sendBroadcast(new Intent(EaseuiConstant.ACTION_GROUP_CHANAGED));
        }
    ...
    public class MyContactListener implements EMContactListener {
        ...
        public void onContactAdded(String username) {
            ...
            broadcastManager.sendBroadcast(new Intent(EaseuiConstant.ACTION_CONTACT_CHANAGED));
...
//                break;
    ...
    public EaseUser getUserInfo(String username) {
        ...
        if (username.equals(EMClient.getInstance().getCurrentUser())) {
            ...
            user.setAvatar(UrlUtil.IMAGE_URL + MyShare.getShared().getString("userHead", ""));
            return user;
        }
        ...
        messageListener = new EMMessageListener() {
            ...
            public void onMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    String em_picturl = message.getStringAttribute(EaseConstant.HEAD_IMAGE_URL, "");
            ...
            public void onCmdMessageReceived(List<EMMessage> messages) {
                for (EMMessage message : messages) {
                    ...
                    if (!easeUI.hasForegroundActivies()) {
//                        if (action.equals(RPEaseuiConstant.REFRESH_GROUP_RED_PACKET_ACTION)){
//                            RedPacketUtil.receiveRedPacketAckMessage(message);
//                            broadcastManager.sendBroadcast(new Intent(RPEaseuiConstant.REFRESH_GROUP_RED_PACKET_ACTION));
//                        }
                    }
            ...
            public void onMessageRecalled(List<EMMessage> messages) {
                for (EMMessage msg : messages) {
                    ...
                    msgNotification.setAttribute(EaseuiConstant.MESSAGE_TYPE_RECALL, true);

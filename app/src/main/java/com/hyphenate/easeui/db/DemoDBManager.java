...
public class DemoDBManager {
    ...
    private DemoDBManager() {
        dbHelper = DbOpenHelper.getInstance(CyApplication.getCyContext().getApplicationContext());
    }
    ...
    synchronized public Map<String, EaseUser> getContactList() {
        ...
        if (db.isOpen()) {
            ...
            while (cursor.moveToNext()) {
                ...
                if (username.equals(EaseuiConstant.NEW_FRIENDS_USERNAME) || username.equals(EaseuiConstant.GROUP_USERNAME)
                        || username.equals(EaseuiConstant.CHAT_ROOM) || username.equals(EaseuiConstant.CHAT_ROBOT)) {
    ...
    synchronized public void clearMessage() {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        if (db.isOpen()) {
            db.execSQL("delete from " + InviteMessgeDao.TABLE_NAME);
        }
    }

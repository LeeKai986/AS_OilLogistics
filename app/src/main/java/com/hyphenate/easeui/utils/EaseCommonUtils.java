...
public class EaseCommonUtils {
    ...
    public static void setUserInitialLetter(EaseUser user) {
        ...
        if (!TextUtils.isEmpty(user.getNick())) {
            String tname = user.getNick();
            PersonInfo userinfo = CyApplication.findUserData(user.getNick());
            if (userinfo != null) {
                tname = userinfo.getRelname();
            }
            letter = new GetInitialLetter().getLetter(tname);
            user.setInitialLetter(letter);
            return;
        }
        if (letter.equals(DefaultLetter) && !TextUtils.isEmpty(user.getUsername())) {
            String tname = user.getUsername();
            PersonInfo userinfo = CyApplication.findUserData(user.getUsername());
            if (userinfo != null) {
                tname = userinfo.getRelname();
            }
            letter = new GetInitialLetter().getLetter(tname);
